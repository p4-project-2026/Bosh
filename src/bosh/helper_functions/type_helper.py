...
def narrow(a: set[str], b: set[str]) -> set[str]:
    ...
    "if one set is 'list<any>' or 'list<UNKNOWN>' and the other set has concrete list types, return the concrete list types; if one set is 'list<UNKNOWN>' and the other set is 'list<any>',"
    "return 'list<UNKNOWN>'; otherwise, return the intersection of the two sets."
    ...
    if is_unknown_list_type(a) and has_list_type(b):
        return get_all_list_types(b)
    if is_unknown_list_type(b) and has_list_type(a):
        return get_all_list_types(a)

    if is_empty_list_type(a) and has_concrete_list_type(b):
        return get_all_list_types(b)
    if is_empty_list_type(b) and has_concrete_list_type(a):
        return get_all_list_types(a)
