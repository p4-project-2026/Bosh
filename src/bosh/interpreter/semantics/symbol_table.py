...
class Symbol_Table(Table[set[str]]):
    ...
    def bind(self, name: str, type_value: set[str]):
        ...
        if type_value == {"list<any>"} and any(t.startswith("list<") and t.endswith(">") for t in current_type):
            vvvprint(f"SymbolTable: Attempting to bind 'list<any>' to variable '{name}' which is currently bound to specific list type '{current_type}', allowing re-binding 'list<any>' to specific list type.")
