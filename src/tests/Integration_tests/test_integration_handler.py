...
def strip_ansi_and_normalize(text):
    """Remove ANSI color codes and normalize line endings"""
...
def test_bosh_code(file_path):
    ...
    # clean the output and compare it to the expected result
    output = output.decode("utf-8").strip()
    output = strip_ansi_and_normalize(output)
    result = strip_ansi_and_normalize(result)
